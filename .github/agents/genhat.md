...
19. **Mindmap state reset strategy (lint-safe)**: Canvas interaction state resets are handled by keyed remount (`<MindMapCanvas key={graph.id} ... />`) rather than `setState` inside `useEffect`; overlay zoom is graph-scoped (`Record<graphId, zoom>`) with default `1.0`, avoiding `react-hooks/set-state-in-effect` violations.
20. **Mindmap node opacity and layer ordering**: Mindmap node rectangles are fully opaque and the SVG graph layer is explicitly above the dotted background layer (`z-10` vs `z-0`), so dots remain visible only around nodes and never on top of node bodies.
21. **Per-task priority**: Models can override their default priority for specific tasks via `[models.task_priorities]` in `models.toml` and the `task_priorities: HashMap<TaskType, u32>` field in `ModelDef`. The registry's `find_for_task()` uses `priority_for_task()` which checks task-specific overrides first. This enables different priority ordering for chat vs podcast_script without duplicating model entries.
...
34. **Workspace system foundation (phase 1)**: Added backend workspace registry/state via `src-tauri/src/workspace.rs` with default workspace bootstrap, stable workspace scope IDs (`workspace:<id>`), per-workspace cache directories, and metadata (`id`, `name`, `nela_path`, `cache_dir`, timestamps). Added Tauri commands in `src-tauri/src/commands/workspace.rs`: `list_workspaces`, `get_active_workspace`, `create_workspace`, `open_workspace`, `set_workspace_file`. `main.rs` now initializes `WorkspaceManager`, uses the active workspace cache `rag` path for `RagPipeline::open`, and registers `WorkspaceState`. `get_workspace_scope` no longer uses CWD; it now returns the active workspace scope from `WorkspaceState`.
35. **Workspace system phase 2 (.nela packaging + app controls)**: Implemented `.nela` save/open plumbing in `src-tauri/src/workspace.rs` using streaming ZIP I/O and schema manifest validation (`manifest.json`, `workspace.json`, optional `frontend_state.json`, plus `rag/`, `audio/`, `podcasts/`, `docs/` subtrees from workspace cache). Added commands in `src-tauri/src/commands/workspace.rs`: `get_workspace_frontend_state`, `save_workspace_frontend_state`, `save_workspace_as_nela`, `save_workspace_nela`, `open_workspace_nela`. Registered these in `main.rs`. Frontend `api.ts` now exposes matching methods. `App.tsx` now mirrors session/mindmap state to backend workspace state and restores from backend first (localStorage fallback), and includes top-bar workspace controls for create/switch/open/save/save-as.
36. **Live RAG pipeline rebinding on workspace switch/open**: `RagPipelineState` is now hot-swappable (`RwLock<Arc<RagPipeline>>`) in `commands/rag.rs` with `active_pipeline()` / `replace_pipeline()` helpers. RAG and podcast commands now resolve the active pipeline via these helpers instead of capturing a fixed Arc. Workspace-changing commands (`create_workspace`, `open_workspace`, `open_workspace_nela`) in `commands/workspace.rs` now reopen `RagPipeline` against the active workspace cache `rag` directory and replace the active pipeline immediately, so retrieval uses the selected workspace without app restart.
37. **Workspace controls moved to left nav**: Workspace create/manage controls are no longer in the top header action row. They now live in `components/SidebarNav.tsx`, directly under the Chats button area, with a dedicated create workspace button and a workspace management popover (select/rename/delete).
38. **Sidebar import/export actions**: Project import/export (`open_workspace_nela` / `save_workspace_nela` flow via frontend callbacks) is now exposed as icon-only bottom-left sidebar actions in `SidebarNav.tsx` (Folder Open for import, Save for export), replacing the previous top-header Open/Save buttons.
...
42. **Windows-safe `.nela` save replacement**: In `src-tauri/src/workspace.rs`, temp-file replacement now uses a cross-platform helper (`replace_file_from_temp`) that removes existing targets before replace and falls back to copy+delete when rename fails. This prevents `.nela.tmp` files from being left as the final artifact on Windows and ensures exports end as `.nela`.